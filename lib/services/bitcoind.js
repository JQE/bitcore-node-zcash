...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
	...
	['getBlockHashesRange', this, this.getBlockHashesRange, 2],
      ...
      node.client.getBlockchainInfo(function(err, response) {
        ...
        if (progress >= self.zmqSubscribeProgress) {			
          ...
          callback(null, true);
			...
			var synced = response.result.blocks - self.startHeight;
		  var info = {
			  blockChainHeight: response.result.blocks,
			  syncPercentage: Math.round(progress*100),
...
Bitcoin.prototype.syncPercentage = function(callback) {
  ...
  this.client.getBlockchainInfo(function(err, response) {
    ...
    var percentSynced = response.result.verificationprogress * 100;
	var commitments = response.result.commitments;
...
/**
 * Will retreuve an array of block hashes within a range of Hieght
 * @param {Number} high - the highest block height we want
 * @param {Number} low - the lowest block height we want
 * @param {Function} callback
 */
 Bitcoin.prototype.getBlockHashesRange = function(high, limit, callback) {
  var self = this;
  var hashes = [];
  var gotError = false;
  
  function oneBlock(index) {
	  if (gotError) { return callback(err);}
	  if (index < high-limit) {
		  return callback(null, hashes);
	  } else {		  
		  self.getBlock(index, function(err, blockObj) {
			if (err) { 
			  gotError = true;
			  return callback(err);
			}
			hashes.push(blockObj.hash);
			return oneBlock(index-1);
		  });
	  }
  }  
  oneBlock(high);
...
Bitcoin.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  callback(new Error("This function doesn't work and shouldn't be called")); 
};
...
Bitcoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
  var tx = null;//self.rawTransactionCache.get(txid);
  if (tx) {
    return setImmediate(function() {
      callback(null, tx);
    });
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  var tx = null;//self.transactionDetailedCache.get(txid);
  
  function getInputAddress(input, callback) {
	if (input.iscoinbase) {
		callback(index);
	} else {
	  self._tryAllClients(function(client, done) {
		client.getRawTransaction(input.prevTxId, 1, function(err, response) {
		  if (err) {
		    return done();
		  }
		  input.address = response.result.vout[input.outputIndex].scriptPubKey.addresses[0];
		  input.value = response.result.vout[input.outputIndex].valueZat;
		  return done();
		});
	  }, callback);
	}
  ...
  function addInputsToTx(tx, result, client) {
    tx.inputs = [];
    tx.inputSatoshis = 0;
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        addInputsToTx(tx, result, client);
		...
		async.forEach(tx.inputs, function(item, done) {
			getInputAddress(item, done);
		}, function(err) {
			callback(null, tx);
		});
	});
